...
UPLOAD_FOLDER = 'imagesC'
UPLOAD_FOLDERP = 'imagesP'
...
def submitInf():
    ...
    result = test.compare_all_concerned_citizen("imagesP/" + S.inf[0])
    print(S.inf[0])
    print(result)
    if result == None:
    ...
    if isinstance(result, str):
        S.result = result
        os.remove("imagesP/" + S.inf[0])

        cnx = test.open_database_connection()
        mycursor = cnx.cursor()

        name = "SELECT child_name FROM innodb.concerned_citizen where image = '" + S.result + "'"
        age = "SELECT age FROM innodb.concerned_citizen where age = '" + S.result + "'"
        gender = "SELECT gender FROM innodb.concerned_citizen where gender = '" + S.result + "'"
        location = "SELECT location FROM innodb.concerned_citizen where location = '" + S.result + "'"
        phone = "SELECT phone FROM innodb.concerned_citizen where phone = '" + S.result + "'"
...
def submitInfC():
    ...
    S.inf[5] = int(request.form['contact'])
    print(S.inf)
    result = test.compare_all_parent("imagesC/" + S.inf[0])
    print(result)
    if result == None:
        return render_template('citizenNoResults.html')
    if isinstance(result, str):
        S.result = result
        os.remove("imagesC/" + S.inf[0])
